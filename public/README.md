...
The app now runs the **hybrid glass/solid** visual direction — see
`DESIGN.md` for full tokens and component rules. Two files carry the entire
reskin (no page/component logic changed):

- `styles/globals.css` — glass treatment on shell surfaces (navbars,
  dashboard hero, quick actions, premium cards), solid high-contrast cards
  on dense data (checklists, wizard forms, requirement detail, admin) to
  protect legibility/performance on lower-end Android hardware.
- `app/layout.tsx` — loads Space Grotesk (display) + Inter (body) and sets
  the PWA theme color to the new brand blue.
